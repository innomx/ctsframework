...
x <- CTS(base_scenario)
outputs <- attr(x$scenario$summarization, "outputs")
outres <- rlang::eval_tidy(outputs, c(x))
outres[[1]]
outres[[2]]
```
...
Each scenario will generate the same outputs as the base scenario.
...
```{r}
dat <- collate("cts_results/2023-01-20", "endpoints/sim-exposure.csv")
```

Now, we can produce a nice table to summarize our results across all 6 scenarios:

```{r}
dat <- dat[scenario %in% get_scenario_table(scgrid)$scenario]
